...
function encryptStarlightPassword(password) {
  const now = Date.now();
  const slice = Math.floor((now - (now % 30000)) / 30000);
  const key = Buffer.from(`${slice}${slice}${slice}${slice}`);
...
async function refreshStarlightAuth({ force = false } = {}) {
  ...
  const response = await fetch(`${STARLIGHT_ORIGIN}/api/keystone/short_term_token/name`, {
    ...
    body: JSON.stringify({
      username: STARLIGHT_USERNAME,
      password: encryptStarlightPassword(STARLIGHT_PASSWORD),
